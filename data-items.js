...
//   personal   … ★ 観察記録票だけの例外（2026-09-23・EX-144 裁定）。**誰に持たせるかを選ぶ**（他は共有の荷）。
//                消耗しない・在庫に数えない・買わない（ギルドには無限にある）。ただし持たせた人のスロットを1つ埋める。
//                ★ 記録票は「声の道具」＝中身が書いた人の主観（SOAP の S）なので、誰に書かせるかがプレイヤーの判断になる。
//   unlockedBy … personal の品が持たせられるようになる条件（体験版＝隊商護衛のあと。本編は図書館のイベント後に変える予定）。
window.masterItems = [
  ...
  { id: "item_obs_sheet", name: "観察記録票", tags: ["観察", "記録", "生物", "生態目録"], note: "観察対象がいる依頼で持たせると、報告書に冒険者ごとの観察メモが追加される。", personal: true, unlockedBy: "quest_caravan_escort" },
