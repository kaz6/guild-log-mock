...
test("報告書の生成が基準と一致する（全依頼 × 編成30 × seed4 ＋ 支給品の軸・10項目）", async () => {
  ...
  const r = compare(baseline.rows, rows, { fields: FIELDS, keyFields: KEY_FIELDS, by: (row) => (row.supplies ? `${row.quest}（支給品）` : row.quest) });
