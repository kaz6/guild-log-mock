...
window.masterQuests = [
  ...
  {
    // ★ 救済クエスト（2026-09-23・EX-144）。出典は Notion「設計：金と在庫と買い出しクエスト」（2026-09-21）。
    //   ★ **費用0・報酬は少し**（報酬8＝近の遠征費10に届かない額＝金策にならない。仮の値）。
    //   ★ **運営不能の直前には特別に優先して出る**。「直前」＝**借金中、または借金2回を使い切った状態**
    //     （裁定 2026-09-22 の書き直し。⚠️ 作者の定義として確定ではない）。
    //   ⚠️ **「たまに出る」の間隔 N は未確定**。決まるまで**直前のとき以外は出さない**（N＝∞として扱う）。
    //     ⚠️ これは金策ループ（未確定）を**一時的に塞いでいる**だけで、防ぎ方を決めたわけではない。
    //   ★ `relief` の旗で分岐する（id のハードコードでは分岐しない）。解放条件は持たない（直前かどうかだけで出る）。
    //   ⚠️ 文面（題名・本文・結末）はすべて仮。チャット側が書き直す待ち。
    id: "quest_relief_odd_jobs",
    title: "町の小口の頼まれごと",
    relief: true,
    category: "生活",
    danger: "低",
    ...
    tags: ["生活", "雑用", "小口"],
    observationTarget: "なし",
    summary: "町の人から、ギルドへ小さな用が回ってきた。手間の割に謝礼は少ないが、断る理由もない。",
    money: { fee: 0, reward: 8 },
    outcomes: { full: ["小口の用を済ませた"], partial: [], fail: [] }
  }
];
