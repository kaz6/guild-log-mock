...
> **正本の所在**（2026-07-26 照合／2026-08-01 更新）：★ **向きは「誰が書くか」で決まる**
> （人・チャットが書くもの＝Notion／Claude Code が生成するもの＝repo／実装から導出できる事実＝コード。
> 詳細は `02_operations/ドキュメント運用規約`）。
> ただし**本文書に対応する Notion ページは未作成**のため、現時点では本ファイルが最新。
> Notion 側にページが作られた時点で、上の判定に従って向きを決める。
