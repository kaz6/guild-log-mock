...
// 行方不明の時計（2026-08-18・EX-070）。★ 値は調整前提でデータ側に持つ。
// 猶予は「行動時間」＝進行中の遠征が存在する間だけ進む時間で、24時間相当。
// ★ demo 倍率（getDemoSpeed）は掛けない（負傷の回復と違い、急かさない時間なので）。
window.masterMissingClock = {
  limitMs: 24 * 60 * 60 * 1000,
  // 段階の閾値（経過÷limit の割合）。★ 数字の残り時間は出さず、この段階だけを見せる。
  // ★ 文言は 2026-09-11（EX-072）に本置き。仮置きの「手掛かりが薄れつつある」「絶望的」は採らない——
  //   あれは現場を見た者の評価で、現場にいない記録係には書けない。書けるのは「事実」と「目の前にいる人の顔」だけ。
  // companionLine の {名前} は app.js 側で実名に置き換える（その者と最後に同じ遠征へ出ていた者。
  //   いなければ受付嬢＝window.masterReceptionist.name。将来 関係値ができたらそこで選び直す）。
  stages: [
    { upTo: 0.4, label: "消息を絶っている" },
    { upTo: 0.8, label: "消息を絶ってから、しばらく経つ", companionLine: "{名前}に焦りが見える" },
    { upTo: 1.0, label: "消息を絶ってから、だいぶ経つ", companionLine: "{名前}の表情は暗い" }
  ]
};
